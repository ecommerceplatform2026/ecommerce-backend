...
namespace Ecommerce.UnitTests.EntityTests
{
    public class DeliveryTests
    {
        ...
        [Fact]
        public void ResetForRetry_WhenException_SetsStatusPendingAndClearsTracking()
        {
            var delivery = CreatePendingDelivery();
            delivery.MarkShipmentCreated("TRACK-1", "ORD-1", 10_000);
            delivery.MarkException("Lost in transit");
            ...
            delivery.CarrierOrderCode.Should().BeNull();
            delivery.ShippingFee.Should().Be(0);
            delivery.Note.Should().Contain("Lost in transit");
            delivery.Note.Should().Contain("[Retry at");
        }

        [Fact]
        public void ResetForRetry_WhenNotException_ThrowsInvalidOperationException()
        {
            var delivery = CreatePendingDelivery();
            delivery.MarkShipmentCreated("TRACK-1", "ORD-1", 10_000);

            Action act = () => delivery.ResetForRetry();

            act.Should().Throw<InvalidOperationException>()
                .WithMessage("*Only Exception deliveries can be retried*");
        }

        [Fact]
        public void ResetForRetry_WhenPending_ThrowsInvalidOperationException()
        {
            var delivery = CreatePendingDelivery();
        ...
        [Fact]
        public void ResetForRetry_WhenDelivered_ThrowsInvalidOperationException()
        {
            var delivery = CreatePendingDelivery();
            delivery.MarkShipmentCreated("TRACK-1", "ORD-1", 10_000);
            delivery.MarkPickedUp();
        ...
        [Fact]
        public void ResetForRetry_WithNullNote_DoesNotThrow()
        {
            var delivery = CreatePendingDelivery();
            delivery.MarkException(null);
