...
namespace Ecommerce.IntegrationTests.Controllers
{
    public class OrdersControllerIntegrationTests : IClassFixture<CustomWebApplicationFactory>
    {
        ...
        [Fact]
        public async Task ReturnOrder_ReturnsOk_WithReturnedStatus()
        {
            var user = User.Create("Return User", "return@example.com", "hash");
            Order order;
            using (var scope = _factory.Services.CreateScope())
            {
                var db = scope.ServiceProvider.GetRequiredService<EcommerceContext>();
                ...
                order = Order.Create(user.Id, 112235, PaymentMethod.COD);
                typeof(Order)
                    .GetProperty(nameof(Order.Status), System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public)!
                    .SetValue(order, OrderStatus.Delivered);
                db.Orders.Add(order);
            ...
            var response = await _client.PostAsync($"/api/orders/{order.Id}/return", null);

            response.StatusCode.Should().Be(HttpStatusCode.OK);
            var result = await response.Content.ReadFromJsonAsync<ApiResponse<ReturnOrderResponse>>();
            result.Should().NotBeNull();
            result!.Success.Should().BeTrue();
            result.Data.Should().NotBeNull();
            result.Data!.Status.Should().Be(nameof(OrderStatus.Returned));
        }
