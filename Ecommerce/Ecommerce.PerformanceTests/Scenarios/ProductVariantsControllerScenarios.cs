...
public sealed class ProductVariantsControllerScenarios
{
    private readonly PerformanceTestFixture _fixture;

    public ProductVariantsControllerScenarios(PerformanceTestFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    {
        return Scenario.Create("product_variants_load", async context =>
        {
            ...
            IResponse listResult = await Step.Run("list", context, async () =>
            {
                var response = await client.GetAsync(
                    $"/api/products/{_fixture.ProductId}/variants",
            ...
            {
                var response = await client.GetAsync(
                    $"/api/products/{_fixture.ProductId}/variants/{_fixture.VariantId}",
                    context.ScenarioCancellationToken);
                return response.IsSuccessStatusCode
                    ? Response.Ok(statusCode: ((int)response.StatusCode).ToString())
                    : Response.Fail(statusCode: ((int)response.StatusCode).ToString());
    ...
    {
        return Scenario.Create("product_variants_stress", async context =>
        {
            var client = _fixture.CreateClient();
    ...
    {
        return Scenario.Create("product_variants_endurance", async context =>
        {
            var client = _fixture.CreateClient();
